...
import { fadeInOut, INavbarData } from '../helpers/helper';
  ...
  [@submenu]="expanded
    ...
    <li *ngFor="let item of data.items" class="sublevel-nav-item">
        <a class="sublevel-nav-link" (click)="handleClick(item)" *ngIf="item.items && item.items.length > 0">
            <mat-icon aria-hidden="false" fontSet="fas" aria-label="" fontSet="fas" fontIcon="fa-circle" class="sublevel-link-icon"></mat-icon>
            <!--<mat-icon aria-hidden="false" fontSet="fas" aria-label="" fontIcon="{{item.icon}}" class="sublevel-link-icon"></mat-icon>-->
            <span class="sublevel-link-text" *ngIf="collapsed">
                {{item.label}}
            </span>
            <mat-icon aria-hidden="false" aria-label="Times icon" 
        ...
        <a class="sublevel-nav-link" 
        	*ngIf="!item.items || (item.items && item.items.length === 0)"
            ...
            >
            <mat-icon aria-hidden="false" aria-label="" fontSet="fas" fontIcon="fa-circle" class="sublevel-link-icon"></mat-icon>
